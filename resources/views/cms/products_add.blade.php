...
@section('cms_header')
@include('utils.cms_header',['url' => 'products/create','title' => '+ Add New Product'])
@endsection
{{------------------------------}}
@section('cms_content')
...
<div class="row">
<div class="col">
<form action="{{url('cms/products')}}" runat="server" method="POST" autocomplete="off" novalidate="novalidate" enctype="multipart/form-data">
@csrf()
<div class="row">
...
<option @if(old('categorie_id')==$category_item['id']) selected="selected" @endif value="{{$category_item['id']}}">{{$category_item['ctitle']}}</option>
@endforeach
</select>
...
<input value="{{old('title')}}" type="text" name="title" id="title" class="form-control origin-text  " placeholder="* Title">
...
<input value="{{old('url')}}" type="text" name="url" id="url" class="form-control target-text  " placeholder="* Url">
...
<div>
    ...
    <div class="card"  > &nbsp; * Upload Image
        <img class="card-img " id="img-pre" height="290px"  src="" alt="">
...
<textarea class="form-control" name="article" id="article" cols="30" rows="10" placeholder="* Article">{{old('article')}}</textarea>
